/**
 * @file mandelbrot_final_palette_pack.cu
 * @brief CUDA Mandelbrot set renderer with multiple color palettes and output options.
 *
 * Supports grayscale and color (PPM) output, smooth coloring, binary/ASCII output, and palette selection.
 *
 * Build:
 *   nvcc mandelbrot_final_palette_pack.cu -o mandelbrot_final
 *
 * Key flags:
 *   --palette {grayscale,hsv,fire,ocean}   (default grayscale)
 *   --color [--smooth] [--binary]          (PPM out when --color)
 *
 * Other flags:
 *   --width/--height/--max-iter/--xmin/--xmax/--ymin/--ymax/--verify/--out
 */
...
struct Args {
  ...
  std::string palette = "grayscale"; ///< Palette selection
...
Options:
  ...
  --palette NAME     Palette: grayscale | hsv | fire | ocean  (default grayscale)
  --verify K         CPU spot-verify K pixels (default 6; 0 disables)
  --out PATH         Output file path (default mandelbrot.out)

Examples:
  %s --width 1920 --height 1080 --color --smooth --binary --palette hsv --out fractal.ppm
  %s --xmin -0.9 --xmax -0.6 --ymin 0.0 --ymax 0.3 --max-iter 1200 --color --palette fire --out zoom.ppm
)", prog, prog, prog);
}
...
static Args parse_args(int argc, char** argv) {
  ...
  for (int i=1; i<argc; ++i) {
    ...
    else if (s == "--palette")    { need(i); a.palette = argv[++i]; }
 ...
 * @param B Output blue channel
 */
__device__ __forceinline__ void palette_hsv_dev(float t, uint8_t& R, uint8_t& G, uint8_t& B) {
...
/**
 * @brief CUDA kernel to compute Mandelbrot set image with palette/color options.
 *
 ...
 * @param palette_id Palette selection
 */
__global__ void mandelbrot_kernel(uint8_t* out, int W, int H,
  ...
  if (mode == 0) {
    ...
    if (it == max_iter) {
      ...
      switch (palette_id) {
        case 0: { // grayscale in color mode (R=G=B)
          uint8_t v = (uint8_t)lrintf(255.0f * t);
          R = G = B = v;
          break;
        }
        case 1: { // hsv rainbow palette
          palette_hsv_dev(t, R,G,B);
          break;
        }
        case 2: { // fire: black->red->orange->yellow->white
          // Warm ramp with non-linear lift in G, gentle B
          R = (uint8_t)lrintf(255.0f * t);
          G = (uint8_t)lrintf(255.0f * sqrtf(t));
          B = (uint8_t)lrintf(64.0f  * t);
          break;
        }
        case 3: { // ocean: deep blue -> teal -> light blue -> white
          R = (uint8_t)lrintf(48.0f  * t);
          G = (uint8_t)lrintf(160.0f * sqrtf(t));
          B = (uint8_t)lrintf(255.0f * t);
          break;
        }
        default: {
          palette_hsv_dev(t, R,G,B);
...
static inline void cpu_color_for_point(double cr, double ci, int max_iter, bool smooth,
  ...
  if (palette_id == 0) {
    uint8_t v = (uint8_t)lround(255.0 * t);
    R = G = B = v;
  ...
  } else if (palette_id == 2) { // fire
    R = (uint8_t)lround(255.0 * t);
    G = (uint8_t)lround(255.0 * std::sqrt(t));
    B = (uint8_t)lround(64.0  * t);
  } else if (palette_id == 3) { // ocean
    R = (uint8_t)lround(48.0  * t);
    G = (uint8_t)lround(160.0 * std::sqrt(t));
    B = (uint8_t)lround(255.0 * t);
  } else {
    float h = std::fmod(0.95f + 10.0f * t, 1.0f);
    float s = 0.8f;
    float v = std::min(1.0f, std::max(0.0f, 0.2f + std::sqrt((double)t)));
...
int main(int argc, char** argv) {
  ...
  // Map palette string to palette_id for device/host logic
  int palette_id = 0;
  if      (a.palette == "grayscale") palette_id = 0;
  else if (a.palette == "hsv")       palette_id = 1;
  else if (a.palette == "fire")      palette_id = 2;
  else if (a.palette == "ocean")     palette_id = 3;
  else {
    std::fprintf(stderr, "Warning: unknown palette '%s', defaulting to 'grayscale'\n",
                 a.palette.c_str());
    palette_id = 0;
  }

  // Print configuration summary
  std::printf("Mandelbrot %dx%d, max_iter=%d, %s, %s, palette=%s -> %s\n",
              a.W, a.H, a.max_iter,
  ...
  if (a.verify > 0) {
    ...
    for (size_t i=0; i<samples.size(); ++i) {
      ...
      if (!a.color) {
        ...
        uint8_t r,g,b;
        cpu_color_for_point(cr, ci, a.max_iter, a.smooth, palette_id, r,g,b);
