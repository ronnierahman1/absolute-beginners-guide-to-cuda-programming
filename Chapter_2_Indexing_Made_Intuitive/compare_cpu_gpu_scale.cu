...
bool compare_results(const float* y_cpu,
                     const float* y_gpu,
                     int N,
                     float tol = 1e-5f,
                     int max_report = 10)
{
    ...
    int reported = 0;
    for (int i = 0; i < N; ++i) {
        float diff = std::fabs(y_cpu[i] - y_gpu[i]);
        if (diff > tol) {
            if (reported < max_report) {
                printf("Mismatch at %d: CPU=%f, GPU=%f, |diff|=%f\n",
                       i, y_cpu[i], y_gpu[i], diff);
                ++reported;
            }
            pass = false;
        }
    ...
    if (!pass && reported < max_report) {
        // If there were more mismatches than we printed, hint at it.
